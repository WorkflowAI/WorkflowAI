...
summary: Explain the process of using deployments (that allow to update prompts without changing code) from the context of a MCP client
---
...
You need to:
- fetch the existing deployments for `agent_id` (TODO: add the MCP tool)
- if not deployments are found, you suggest to the user to create a new deployment via the web app using the url `https://workflowai.com/agents/<agent_id>/deployments`
