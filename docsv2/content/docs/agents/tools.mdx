...
For example, to use the `@search-google` tool, you can add the following to your agent prompt:

```python
messages = [
{
  "role": "system",
  "content": "Use @search-google to find the weather in {{location}}."
},
]
```
...
- `@search-google` makes a web search using Google
...
TODO: make a table (including pricing) with all the tools using the `/v1/tools/hosted` endpoint

### List hosted tools programmatically

You can list all the hosted tools programmatically:
- by calling the `list_hosted_tools` tool from the MCP server (returns pricing information)
