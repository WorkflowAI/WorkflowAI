...
export function MajorVersionCombobox(props: MajorVersionComboboxProps) {
  const { majorVersions, selectedMajorVersion, onSelectMajorVersion } = props;
  ...
  const currentSearchValue = useMemo(
    () => searchValueForVersion(selectedMajorVersion, newestMajorVersion),
    [selectedMajorVersion, newestMajorVersion]
  );
  ...
  }, [selectedMajorVersion, open, currentSearchValue]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <div
          className={cx(
            'flex flex-row py-1.5 pl-3 pr-2.5 cursor-pointer items-center border border-gray-200/50 rounded-[2px] text-sm font-normal font-lato truncate min-w-[75px] justify-between h-[36px]',
            open
                ...
                {filteredMajorVersions.map((version) => (
                  ...
                  >
                    <MajorVersionComboboxEntry
                      ...
                      newestMajorVersion={newestMajorVersion}
                      trigger={false}
                      isSelected={version.major === selectedMajorVersion?.major}
                    />
                  </CommandItem>
                ))}
              </CommandGroup>
