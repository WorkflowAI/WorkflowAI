...
async def list_available_models(
    ...
    - The `reasoning` boolean field indicating whether the model supports reasoning mode with step-by-step reasoning content
    - Quality index, cost information, and release dates for comparison
    - Filtering options based on agent requirements

    Use the `reasoning` field to identify models that support reasoning mode for complex problem-solving tasks.
    </returns>"""
...
async def fetch_run_details(
    ...
    CONCEPT: This manual 40+ line docstring could be auto-generated from MCPRun field descriptions.

    The decorator would extract field descriptions from MCPRun.model_fields and generate
    this documentation automatically, ensuring it always matches the actual return structure.
    <<@auto_document_from_schema(return_type=MCPRun)>>
    </returns>"""
    service = await get_mcp_service()
