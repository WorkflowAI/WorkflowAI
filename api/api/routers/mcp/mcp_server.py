...
# Create a placeholder function that will be used to generate the dynamic docstring
# This will be called at runtime to discover documentation
def _search_documentation_description():
    # For now, return a static description. The actual dynamic discovery
    # will happen inside the search_documentation implementation
    return """Search WorkflowAI documentation OR fetch a specific documentation page.
     ...
     2. Direct navigation mode ('page' parameter): Fetch the complete content of a specific documentation page. Use direct navigation mode when you want to read the full content of a specific page. Example: 'page': 'reference/authentication'
    </when_to_use>

     <available_pages>
     Documentation pages are dynamically discovered from the docsv2 directory. Common categories include:
     - Getting Started (index, why-workflowai, self-hosting, pricing, etc.)
     - Use Cases (chatbot, new_agent, classifier, image processing, etc.)
     - API Reference (authentication, api-errors, supported models, etc.)
     - Quickstarts (no-code, openai integration guides, etc.)
     - Playground, Observability, Inference, Deployments, Evaluations, Agents, and more

     To see all available pages, use the tool and check the error message if you provide an invalid page name.
     </available_pages>
...
@_mcp.tool(description=_search_documentation_description())
