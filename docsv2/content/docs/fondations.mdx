...
Technical details:
- all requests are proxied by WorkflowAI, then sent to a AI provider. 
...
Deployments allow to update an agent's prompt or model without changing the code. Learn more about deployments by reading the [Deployments](/deployments) section.

## `/v1/chat/completions`

### Parameters

Building a AI agent is the process of picking the right value for each parameter of the `/v1/chat/completions` API. Let's go through each parameter one by one.

```python
completion = client.create.completion( or # client.beta.chat.completions.parse for structured outputs
    model="..",
    messages=[...],
...
`messages`: the messages to send to the model. the messages are a list of dictionaries, each dictionary containing a role and a content. the role can be "user", "assistant", or "system". the content can be a string, or a list of strings.
...
- [input variables](/observability/input-variables) allows to use Jinja2 template syntax to separate static instructions from dynamic data. for example:
...
see `input` parameter below on how to pass variables to the LLM. Note that the template rendering is done server-side by WorkflowAI, so the client does not need to render the template.

Learn more about input variables by reading the [Input Variables](/observability/input-variables) section.

- [deployments](/deployments). When using deployments, the `messages` parameter could be empty because the messages are stored on WorkflowAI directly, and added automatically to the request. `messages = []` is valid, note that the `messages` parameter is required by OpenAI SDKs, so `messages = None` is not valid.
...
`input`: (required when using [input variables](/observability/input-variables)) provides variables to the LLM.

```python
completion = client.create.completion(
