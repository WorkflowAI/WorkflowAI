...
Use `mcp_workflowai_get_run` to examine the details of the problematic run(s):
...
Use `mcp_workflowai_search_documentation` to research relevant errors and debugging methods:
...
- Consider the specific content of the prompt input when tailoring improvements
- Avoid mentioning "JSON" in the improved prompt
- Do not add markdown formatting to the improved prompt
- Only add tool capabilities (using "@`tool_name`" format) if explicitly requested by the user and the tool is available in the provided tool descriptions
