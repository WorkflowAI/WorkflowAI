...
### Reusing Inputs Across Experiments

When running comparative experiments or testing different prompts with the same input data, use `run_query` instead of duplicating input variables to save tokens and costs:

**❌ Inefficient - Duplicates input data:**
```json
{
  "inputs": [{"variables": {"large_content": "..."}}],
  "prompts": [...]
}
```

**✅ Efficient - Reuses existing input:**
```json
{
  "run_query": "SELECT * FROM runs WHERE id = 'run_12345'",
  "prompts": [...]
}
```

This approach:
- Saves tokens by avoiding input duplication
- Ensures perfect input consistency across experiments
- Reduces experiment costs for comparative testing
