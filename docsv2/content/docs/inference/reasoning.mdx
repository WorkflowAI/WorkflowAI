...
The response includes reasoning capability in the `supports` array:
...
- `supports` array: Look for "reasoning" in this array to identify reasoning-capable models
- Models with "reasoning" in their supports array can generate step-by-step reasoning content
- Use these models for complex problem-solving tasks

<Callout type="warning">
**TODO**: The reasoning capability exposure is currently in architectural design phase. Guillaume will implement the actual reasoning detection logic in the MCP model responses and the ability to access reasoning content through MCP tools.
</Callout>
