...
description: Maintain conversation context within a single chat session using reply_to_run_id
---

import { Callout } from "fumadocs-ui/components/callout"

Learn how to maintain conversation context within a single chat session without manually managing message history.
...
<Callout type="info">
**Chat Continuity vs. Persistent Memory:** Chat continuity maintains context within a single conversation using `reply_to_run_id`. For remembering facts across separate conversations, see [Persistent Memory](/docs/agents/memory/persistent-memory).
</Callout>
...
conversation_id = response.id # [!code highlight]
...
response = client.chat.completions.create(
    ...
    extra_body={
        "reply_to_run_id": conversation_id # [!code highlight]
    }
)
```
